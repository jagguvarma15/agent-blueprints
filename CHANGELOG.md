...
### Added
- **Agent kernel** (`core/`). A first-class normalized core every blueprint composes onto — Step / Engine / Control-Policy / Run-State / Ports / Cross-cutting — documented at all five levels, plus the composition **IR** (`core/spec/ir.schema.json` + a worked `example.yaml`) a selection compiles to, and framework-agnostic Python + TypeScript interface stubs (`core/interfaces/`). No runtime: interfaces here, implementations in agent-deployments.
- **Five-level structured entry model.** Every entry now carries YAML frontmatter as its single source (replacing the `metadata.json` sidecar; the generator dual-reads, so `metadata.json` still works as a fallback) with a new `scale` facet (prototype → enterprise) and `kind`. The depth levels are Concepts (`overview.md`) → Architecture (`architecture.md`) → Flow (`flow.md`) → Design (`design.md`) → Implementation (`implementation.md`), each pairing a machine-readable `yaml level=…` block with prose. `react` is migrated as the fully-worked reference (architecture + flow + machine blocks + an `ir_fragment`); remaining entries keep their existing levels and are migrating to the full set.
- **Observability & Tracing foundation** (`foundations/observability-and-tracing.md`) — span-per-step, OpenTelemetry GenAI conventions, trajectory evaluation.
- **Durability & State foundation** (`foundations/durability-and-state.md`) — the checkpoint-vs-durable-execution distinction, the durability tiers, idempotency, pause/resume.
...
### Changed
- Entries migrated from `metadata.json` sidecars to single-source frontmatter; `meta/validate-metadata.js` now reads frontmatter (falling back to `metadata.json`), recognizes the `architecture`/`flow` level files, and emits additive `scale` + `ir_fragment_ref` keys. No `schema_version` bump — additive only, so the agent-deployments consumer is unaffected.
