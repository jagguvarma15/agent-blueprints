...
| Worker hallucinates a tool call | Tool-use registry catches it (see [tool-use](../tool-use/design.md)); worker reports error to supervisor |
...
- **+ [Plan & Execute](../plan-and-execute/overview.md):** Supervisor generates a plan, delegates steps to agents. The plan is the supervisor's decomposition surface; workers execute steps.
...
- **+ [Human in the Loop](../human-in-the-loop/overview.md):** Supervisor or specific worker proposes a high-stakes action; human approves before commit. Often the right termination for multi-agent systems acting on the world.
