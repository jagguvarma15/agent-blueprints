...
**Evolves from:** [Prompt Chaining](../prompt-chaining/overview.md) + [Tool Use](../../primitives/tool_use/overview.md) — adds explicit compensators per step and a state machine that knows how to unwind.
...
- Steps don't need to unwind on failure (e.g., everything is best-effort logging). A simple [Prompt Chain](../prompt-chaining/overview.md) is lighter.
- The process is a single LLM-mediated tool call sequence with no durable side effects — use [Tool Use](../../primitives/tool_use/overview.md) or [ReAct](../react/overview.md).
