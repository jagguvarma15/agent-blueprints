...
This document traces how the [Event-Driven pattern](./overview.md) evolves from the [Tool Use pattern](../tool-use/overview.md), and what it evolves into when latency, throughput, or windowing requirements grow.
...
|---------|-----------------|
| [Multi-Agent (flat)](../multi-agent/overview.md) | Multiple specialized consumers on different event types or partition keys; each one is its own event-driven agent. |
