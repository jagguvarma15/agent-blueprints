...
class ParallelCalls:
    ...
    def run(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            futures = {pool.submit(self._call_branch, i, chunk, branch_prompt): i for i, chunk in enumerate(chunks)}
        ...
        combined = "\n\n---\n\n".join(f"[Part {r.index + 1}]\n{r.output}" for r in branch_results if not r.error)
